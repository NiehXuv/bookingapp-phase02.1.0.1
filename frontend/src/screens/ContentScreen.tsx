...
import { View, Text, Image, TextInput, TouchableOpacity, ScrollView, Dimensions, StyleSheet, Modal, Pressable, Share as RnShare, ActivityIndicator, Alert, RefreshControl, ImageBackground } from 'react-native';
import { useAuth } from '../context/AuthContext';
import { Search, Bell, Heart, MessageCircle, MoreHorizontal, Share2, Pin as PinIcon, UserPlus, Flag, Play, ArrowLeft } from 'lucide-react-native';
...
const ContentScreen: React.FC = () => {
	...
	const { setContentPool, addToContentPool, contentScreenData, setContentScreenData } = useContentContext();
	...
	// Track if we're currently restoring from context to prevent saving loop
	const isRestoringFromContext = useRef(false);
	// Track previous values to prevent unnecessary saves
	const prevContentRef = useRef<ContentItem[]>([]);
	const prevActiveTabRef = useRef<string>('For you');
	const prevSeenContentIdsRef = useRef<Set<string>>(new Set());
	
	// Restore state from context when screen comes into focus (but not on first mount)
	useFocusEffect(
		React.useCallback(() => {
			if (isFirstMount.current) {
				isFirstMount.current = false;
				return; // Skip on first mount, let useEffect handle it
			}
			
			if (contentScreenData && contentScreenData.content.length > 0) {
				// Only restore if the context data is different from current state
				// Compare using refs to avoid dependency on current state
				const contentChanged = JSON.stringify(prevContentRef.current) !== JSON.stringify(contentScreenData.content);
				const tabChanged = prevActiveTabRef.current !== contentScreenData.activeTab;
				const currentSeenIds = Array.from(prevSeenContentIdsRef.current).sort();
				const contextSeenIds = contentScreenData.seenContentIds.sort();
				const seenIdsChanged = JSON.stringify(currentSeenIds) !== JSON.stringify(contextSeenIds);
				
				if (contentChanged || tabChanged || seenIdsChanged) {
					console.log('📦 Restoring ContentScreen state from context');
					isRestoringFromContext.current = true;
					
					// Update refs first to prevent save loop
					prevContentRef.current = contentScreenData.content;
					prevActiveTabRef.current = contentScreenData.activeTab;
					prevSeenContentIdsRef.current = new Set(contentScreenData.seenContentIds);
					
					// Then update state
					setContent(contentScreenData.content);
					setActiveTab(contentScreenData.activeTab);
					setSeenContentIds(new Set(contentScreenData.seenContentIds));
					setIsLoading(false);
					setError(null);
					
					// Reset flag after a short delay
					setTimeout(() => {
						isRestoringFromContext.current = false;
					}, 100);
				}
			}
		}, [contentScreenData])
	...
	// Save state to context whenever it changes (but not during restoration)
	useEffect(() => {
		// Skip saving if we're currently restoring from context
		if (isRestoringFromContext.current) {
			return;
		}
		
		// Check if data actually changed before saving
		const contentChanged = JSON.stringify(content) !== JSON.stringify(prevContentRef.current);
		const tabChanged = activeTab !== prevActiveTabRef.current;
		const seenIdsChanged = JSON.stringify(Array.from(seenContentIds).sort()) !== JSON.stringify(Array.from(prevSeenContentIdsRef.current).sort());
		
		if ((content.length > 0 || seenContentIds.size > 0) && (contentChanged || tabChanged || seenIdsChanged)) {
			prevContentRef.current = content;
			prevActiveTabRef.current = activeTab;
			prevSeenContentIdsRef.current = new Set(seenContentIds);
			
			setContentScreenData({
				content,
				activeTab,
				seenContentIds: Array.from(seenContentIds), // Convert Set to Array for storage
			});
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [content, activeTab, seenContentIds]);
	...
	useEffect(() => {
		// Only fetch if we don't have cached data or if tab changed
		const hasCachedData = contentScreenData && contentScreenData.content.length > 0 && contentScreenData.activeTab === activeTab;
		
		if (!hasCachedData && (content.length === 0 || activeTab !== prevActiveTab.current)) {
			console.log('🔄 No cached data or tab changed, fetching content...');
			fetchContent();
			prevActiveTab.current = activeTab;
		} else if (hasCachedData && content.length === 0) {
			// Restore from cache if we have it but content is empty
			console.log('📦 Restoring from cache on mount');
			setContent(contentScreenData.content);
			setSeenContentIds(new Set(contentScreenData.seenContentIds));
...
const styles = StyleSheet.create({
	...
	container: {
		flex: 1,
		paddingBottom: 20, // Reduced padding to avoid excessive blank space
	},
	tabsRow: {
		flexDirection: 'row',
		alignItems: 'center',
